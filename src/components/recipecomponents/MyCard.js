...
const MyCard = ({
  ...
  const renderAddLikeOnNotify = ({ lsLiked }) => {
    return (
      <div className="flex ">
    ...
    return (
      <div className="absolute bottom-0 group-hover:bottom-4 inset-x-1 flex justify-center opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
        <ButtonPrimary
          ...
          fontSize="text-xs"
          sizeClass="py-2 px-4"
          onClick={() => notifyAddLike({ isLiked:!isLiked })}
        >
...
//   };
  ...
  return (
    <>
      ...
      >
        <Link to={`/recipes/${data.rno}`} className="absolute inset-0"></Link>

        <div className="relative flex-shrink-0 bg-slate-50 dark:bg-slate-300 rounded-3xl overflow-hidden z-1 group">
          <Link to={`/recipes/${RECIPES.rno}`} className="block">
            <img src={data.thumbPath}/>
          </Link>

          {/* <ProductStatus status={status} /> */}

          <LikeButton liked='false' className="absolute top-3 right-3 z-10" />
