...
export default function ComboBox() {
    ...
    useEffect(() => {
        RecipeInit(recipeList)
    },[])

    const initArr = []
    const RecipeInit = (recipeList) => {
        if(recipeList.length > 5) {
            // 레시피 개수를 5개까지 잘라내고 최 하단에 {label: ... 더보기} 배열 초기화 
            /* for(let i=0; i<5; i++) {
                initArr.push(recipeList[i]);
            }
            initArr.push({label : "...더보기"}) */
            // Splice로 처리해보기
            console.log(recipeList.splice(0,5));
            initArr = recipeList.splice(0,5);
            initArr.push({label : "...더보기"})
        }
    ...
    console.log(params)
    //renderInput 의 params는 함수를통해 TextField(입력란)으로부터 입력받은 값을  반환?
    //입력을 렌더링합니다.
    return (
        <Autocomplete
            disablePortal
...
const title = "레시피 예시 1"
const writer = "작성자 예시 1"
const Kcal = "_____"
const searchParam = title + ' ' + writer + '' + Kcal;
// 비동기 통신으로부터 받아온 List 배열 객체.
let recipeList = [
    { label: searchParam, year: 1994 },
